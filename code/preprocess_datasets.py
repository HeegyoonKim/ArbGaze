...
def preprocess_mpii(mpii_dict):
    ...
    for subject in subjects:
        ...
        # Visualization
        for i in range(len(to_write['pixels'])):
            if i % 30 == 0:
                img = to_write['pixels'][i]
                to_visualize = img.copy()
                pw = vector_to_pitchyaw(-to_write['labels'][i].reshape(3,1).T).flatten()
    ...
    for subject in subjects:
        ...
        for i in range(len(gaze_directions)):
            ...
            for j in range(len(imgs)):
                ...
                add('pixels', norm_imgs)
                add('labels', norm_gazes.astype(np.float32))
